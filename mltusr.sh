#!/bin/bash
clear
echo "# Script 1, Pol Tarragó Gargallo"
if [ "$(id -u)" != "0" ]; then
	echo "Executa aquest script com a root (o utilitzant sudo)."
	exit 1
fi
...
if [[ -e usuaris.ods ]]
then
	echo "S'ha descarregat el fitxer correctament"
else
	echo "ERROR: No s'ha pogut descarregar el fitxer"
	exit 2
fi
...
echo -n "S'ha finalitzat el script correctament!"
exit 0
